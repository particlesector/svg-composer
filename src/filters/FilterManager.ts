...
export class FilterManager {
  ...
  private _createGrayscalePrimitives(amount: number): FilterPrimitive[] {
    ...
    const matrix: ColorMatrixPrimitive = {
      ...
      in: 'SourceGraphic',
      values: [
        r + (1 - r) * (1 - amount), g * amount, b * amount, 0, 0,
        r * amount, g + (1 - g) * (1 - amount), b * amount, 0, 0,
        r * amount, g * amount, b + (1 - b) * (1 - amount), 0, 0,
        0, 0, 0, 1, 0,
      ],
    };
    return [matrix];
  ...
  private _createSepiaPrimitives(amount: number): FilterPrimitive[] {
    ...
    const matrix: ColorMatrixPrimitive = {
      ...
      in: 'SourceGraphic',
      values: [
        0.393 + 0.607 * (1 - amount), 0.769 * amount, 0.189 * amount, 0, 0,
        0.349 * amount, 0.686 + 0.314 * (1 - amount), 0.168 * amount, 0, 0,
        0.272 * amount, 0.534 * amount, 0.131 + 0.869 * (1 - amount), 0, 0,
        0, 0, 0, 1, 0,
      ],
    };
    return [matrix];
  ...
  private _createVintagePrimitives(intensity: number): FilterPrimitive[] {
    ...
    const primitives: FilterPrimitive[] = [
      ...
      // Apply warm sepia-like toning
      {
        ...
        in: 'desaturated',
        values: [
          1 + 0.1 * intensity, 0.05 * intensity, 0, 0, 0.02 * intensity,
          0.05 * intensity, 1, 0.02 * intensity, 0, 0,
          0, 0.02 * intensity, 1 - 0.1 * intensity, 0, -0.02 * intensity,
          0, 0, 0, 1, 0,
        ],
        result: 'toned',
      } as ColorMatrixPrimitive,
